...
Web 결과는 file 주소로 직접 열지 않는다. Builds/Web을 정적 Web 서버로 제공하고 새 게임, 자원 생산, 집 기초·골조·완성, 집 완료 뒤 정원 기초·골조·완성, 새로고침 뒤 복구를 직접 확인한다. 정원 완료 뒤에는 다음 생산 주기에서 구름솜과 이슬만 각각 2개 증가하는지도 확인한다. 서버는 압축된 `.gz` 산출물에 `Content-Encoding: gzip`을 반환해야 한다.
